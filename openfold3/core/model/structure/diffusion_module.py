...
class SampleDiffusion(nn.Module):
    """
    Implements AF3 Algorithm 18, with optional coordinate-space bias
    potentials applied inside the diffusion loop.
    """
    ...
    def forward(
        ...
        Args:
            ...
            _mask_trans:
                Whether to mask the output of the transition layer
            potentials:
                Optional list of openfold3.core.metadynamics.base.Potential
                instances. Each is evaluated post-denoising; its gradient is
                added to the DDIM direction and (if stateful) it is invoked
                once per step via on_step_end to deposit hills/etc.
        Returns:
        ...
        for tau, c_tau in enumerate(noise_schedule[1:]):
            ...
            # ------------------------------------------------------------
            # Metadynamics-like coordinate-space bias.
            # The energy gradient on xl_noisy is added to the DDIM
            # direction. dt is negative, so a positive ∇V contributes a
            # negative update (i.e., descent on V = repulsion from past
            # hill centers).
            # ------------------------------------------------------------
            if potentials:
                bias_grad = torch.zeros_like(xl_noisy)
                for p in potentials:
                    bias_grad = bias_grad + p.gradient(
                        coords=xl_noisy,
                        batch=batch,
                        step_idx=tau,
                        noise_level=t,
                ...
                if tau % 20 == 0:
                    ...
                    bias_mag = bias_grad.abs().max().item()
                    print(
                        f"[METAD-DIAG] step={tau} t={t.item():.3f} "
                        f"bias_grad.max={bias_mag:.4e} "
                        f"delta.max={delta_mag:.4e} "
                        f"ratio={bias_mag/max(delta_mag, 1e-12):.4e}",
                        flush=True,
                    )
                # >>> END <
                delta = delta + bias_grad

            dt = c_tau - t
            xl = xl_noisy + self.step_scale * dt * delta

            # ------------------------------------------------------------
            # State update for stateful potentials (e.g. hill deposition).
            # ------------------------------------------------------------
            if potentials:
                for p in potentials:
                    p.on_step_end(
